...
const verifyJwt = async jwtToken => {
  try {
   // logger.info(`Input token is  ${jwtToken}`)
    passport.initialize();
    passport.use('jwt', jwtStrategy);

    const refreshTokenDoc = await tokenService.verifyToken(jwtToken, 'refresh');
    //logger.info(`Token User is  ${refreshTokenDoc.user}`)
